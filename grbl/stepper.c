...
#ifdef ADAPTIVE_MULTI_AXIS_STEP_SMOOTHING
    st.counter_x += st.steps[A_AXIS];
#else
    st.counter_x += st.exec_block->steps[A_AXIS];
#endif
    if (st.counter_x > st.exec_block->step_event_count)
    {
...
#ifdef ADAPTIVE_MULTI_AXIS_STEP_SMOOTHING
    st.counter_y += st.steps[B_AXIS];
#else
    st.counter_y += st.exec_block->steps[B_AXIS];
#endif
    if (st.counter_y > st.exec_block->step_event_count)
    {
...
#ifdef ADAPTIVE_MULTI_AXIS_STEP_SMOOTHING
    st.counter_z += st.steps[C_AXIS];
#else
    st.counter_z += st.exec_block->steps[C_AXIS];
#endif
    if (st.counter_z > st.exec_block->step_event_count)
    {
